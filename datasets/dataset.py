...
class ImageDataTrain(data.Dataset):
    def __init__(self, data_root, data_list):
        ...
        # sal_root:根目录，图片(images)和真值(masks)的根目录
        # sal_list:所有要训练的图片的清单，如'/images/img.jpg'
    ...
    def __getitem__(self, item):
        ...
        depth_name = self.sal_list[item % self.sal_num].split()[2]
        sal_image = load_image(os.path.join(self.sal_root,im_name))
        sal_label = load_sal_label(os.path.join(self.sal_root,gt_name))
        sal_depth = load_depth_image(os.path.join(self.sal_root, depth_name))
...
class ImageDataTest(data.Dataset):
    ...
    def __getitem__(self, item):
        image, image_size = load_image_test(os.path.join(self.data_root, self.image_list[item][0]))
        depth = load_depth_image(os.path.join(self.data_root, self.image_list[item][1]))
        image = torch.Tensor(image)
        depth = torch.Tensor(depth)

        return {'image':image,'depth':depth,'name':self.image_list[item % self.image_num],'size':image_size}
